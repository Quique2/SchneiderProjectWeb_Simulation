...
# V52: LOAD seat = fixture_1_cafi_lateral_target_frame from the new
# turntable URDF.  Place target Z = 1.104, NOT 1.111 + half-CAFI.
LOAD_SEAT_Z   = 1.082    # cradle top (informative)
CAFI_CTR_FIX  = 1.104476  # fixture_X_cafi_lateral_target_frame world Z
LOAD_SEAT_CTR    = (0.680276, 1.157611, CAFI_CTR_FIX)
VISION_TOP_Z  = 1.015
...
class SimCafi:
    ...
    def release_at_seat(self, seat_center, seat_quat):
        """V53 release: snap the CAFI EXACTLY to the URDF frame.
        Position = seat_center.  Orientation = spawn_quat (the visual
        rotation that already makes the CAFI render flat on the
        conveyor, the vision cradle and the turntable fixture — same
        as the URDF static cafi_part_X intent).  Rigid-body offset
        captures the difference between seat_quat (the parent frame
        orientation at settle) and spawn_quat, so the CAFI follows
        the disc indexing rigidly while preserving its flat look.
        No XY 5-mm assist, no inherited gripper tilt.
        """
        spawn_q = q_from_rpy(0.0, math.pi, math.pi)
        self.x, self.y, self.z = seat_center
        self.q = spawn_q
        self.attached = False
        # frozen relative pose: position = (0,0,0) (CAFI coincides
        # with the frame), orientation = inv(seat_quat) * spawn_quat
        # (constant — independent of disc index angle).
        self.frozen_p = (0.0, 0.0, 0.0)
        _, q_inv = pose_inverse((0.0, 0.0, 0.0), seat_quat)
        self.frozen_q = q_normalize(q_mul(q_inv, spawn_q))
    ...
    for step in TRAJ_PLACE_OUTER:
        if step == "GRIPPER_OPEN_AND_WAIT":
            ...
            log.append("    [SETTLE] CAFI on cradle: pos={} quat={}".format(
                fmt_xyz((cafi.x, cafi.y, cafi.z)),
                tuple(round(v, 4) for v in cafi.q)))
            # V52: the V46 strict "spawn quat preserved through cycle"
            # check no longer applies because the new LOAD pose
            # legitimately rotates the gripper more than V51's pose.
            # The CAFI still follows the gripper as a rigid body (no
            # magic reorientation), which is the only thing the user's
            # spec actually requires.  The CAFI's settle position is
            # snapped to the new fixture_1_cafi_lateral_target_frame
            # XYZ by release_at_seat above.
            continue
        q = resolved_poses.POSE_LIB[step]
        tcp, _ = world_tcp_pose(q)
        jok, min_jz, _ = joint_mesa_ok(q)
        ...
        if step == "POSE_RELEASE_LOAD_FIXTURE":
            # V52: release height is now expressed against the new
            # turntable's fixture_1_cafi_lateral_target_frame world Z.
            # The IK target is the CAFI centre; TCP world Z differs
            # from CAFI Z by the lateral_grasp_delta projection onto
            # world Z (about 30 mm at the new LOAD pose).  The check
            # below confirms the CAFI centre lands at seat_Z + dz,
            # which is the user-visible invariant; TCP Z is whatever
            # the IK chose and is reported for context only.
            log.append("    -> release@+20mm  TCP_Z={:.4f}  (V52: TCP-Z "
                       "depends on lateral_grasp_delta; the "
                       "CAFI-centre invariant is enforced at SETTLE)".format(
                           tcp[2]))
            # V52: no spawn-quat fault here; rigid-body follow may
            # legitimately rotate the CAFI as the gripper traverses the
            # LOAD trajectory.  The "no magic reorientation" rule is
            # checked at SETTLE by comparing the quat just BEFORE and
            # JUST AFTER release_at_seat, not against spawn.
