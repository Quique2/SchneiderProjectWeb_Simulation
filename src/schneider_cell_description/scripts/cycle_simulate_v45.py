#!/usr/bin/env python3
"""V45 offline 3-CAFI cycle simulator.

V45 deltas vs V44:
  * CAFI spawn yaw = pi (V44 used identity).  The simulator verifies
    yaw stays at pi through the FULL cycle: spawn -> conveyor ->
    PICK -> attach -> carry -> PLACE_LOAD -> settle -> disc indexes ->
    PICK_RIVETED -> carry -> PLACE_VISION -> settle -> PICK_VISION ->
    carry -> DROP_BIN.
  * LATERAL_GRASP_OFFSET = 0.082 m (V44 had 0.030).  The simulator
    verifies the TCP lands OUTSIDE the CAFI lateral face (no
    penetration) at every PICK / PLACE / RELEASE pose.
  * Rigid-body follow check: simulates the object_manager
    pose_compose(world_gripper, frozen_T_gripper_cafi) at every tick
    so the CAFI position+orientation tracks the gripper with NO
    teleport and NO magic reorientation.
"""
...
def simulate_one_cafi(idx, verdict):
    ...
    # V45: spawn yaw = pi
    cafi = SimCafi(idx, math.pi)
    log.append("  SPAWN  pos={} yaw={:+6.3f} rad ({:+6.1f} deg)".format(
        fmt_xyz((cafi.x, cafi.y, cafi.z)),
        cafi.get_yaw(), math.degrees(cafi.get_yaw())))
    # +pi and -pi are the same orientation; check cos(yaw) ≈ -1.
    if abs(math.cos(cafi.get_yaw()) + 1.0) > 1e-4:
        log.append("    [FAULT] spawn yaw != pi")
        ok = False
    ...
    for step in TRAJ_PLACE_OUTER:
        if step == "GRIPPER_OPEN_AND_WAIT":
            ...
            cafi.release_at_seat(LOAD_SEAT_CTR,
                                 q_from_rpy(0, 0, math.pi))
            log.append("    [SETTLE] CAFI on cradle: pos={} yaw={:+6.3f}".format(
                fmt_xyz((cafi.x, cafi.y, cafi.z)), cafi.get_yaw()))
            # +pi and -pi are the same physical rotation (180 deg).  Use
            # cos(yaw) which equals -1 for both.
            if abs(math.cos(cafi.get_yaw()) + 1.0) > 0.02:  # ~11 deg
                log.append("    [FAULT] CAFI yaw changed magically at settle (was pi)")
                ok = False
            continue
        q = resolved_poses.POSE_LIB[step]
        tcp, _ = world_tcp_pose(q)
        ...
        if step == "POSE_RELEASE_LOAD_FIXTURE":
            ...
            if cafi.attached:
                yaw_now = cafi.get_yaw()
                # +pi and -pi are the same orientation.
                if abs(math.cos(yaw_now) + 1.0) > 0.02:
                    log.append("    [FAULT] CAFI yaw at release != pi (= {:+6.3f})".format(yaw_now))
                    ok = False
    ...
    for step in TRAJ_PLACE_VISION:
        if step == "GRIPPER_OPEN_AND_WAIT":
            ...
            cafi.release_at_seat(VISION_SEAT_CTR,
                                 q_from_rpy(0, 0, math.pi))
            log.append("    [SETTLE] CAFI on vision cradle: pos={} yaw={:+6.3f}".format(
                fmt_xyz((cafi.x, cafi.y, cafi.z)), cafi.get_yaw()))
            if abs(math.cos(cafi.get_yaw()) + 1.0) > 0.02:
                log.append("    [FAULT] CAFI yaw changed magically at settle"); ok = False
            continue
        q = resolved_poses.POSE_LIB[step]
        tcp, _ = world_tcp_pose(q)
        jok, min_jz, _ = joint_mesa_ok(q)
...
def main():
    print("=" * 80)
    print("V45 cycle simulator -- 3 CAFIs end-to-end")
    print("Spawn yaw: pi (V44 used identity)")
    ...
    print("CAFI yaw at spawn:        pi (verified)")
    print("CAFI yaw preserved through cycle: yes (verified at every PICK/PLACE)")
