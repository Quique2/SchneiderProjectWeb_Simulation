...
# ---------------------------------------------------------------------------
@case("F. Cycle — V54 simulator: 0 watchdogs, 0 FAULTs, 0 collisions")
def case_f():
    import subprocess
    out = subprocess.check_output(
        ["python3", os.path.join(HERE, "cycle_simulate_v54.py")],
    ...
    for line in out.split("\n"):
        if "SETTLE" in line and "cradle" in line:
            print("  " + line.strip())
            break
    print("  cycle_simulate_v54.py: 3/3 OK, 0 FAULTs / 0 watchdogs / 0 collisions")


# ---------------------------------------------------------------------------
@case("G. LOAD/RIVETED gripper tilt (V54 reach-limited budget)")
def case_g():
    """V54: PLACE / PICK_RIVETED / RELEASE must be tighter than V53's
    ~45° tilt — V54's iterative lateral-shift solver achieves ~20°
    tilt at the contact poses (the lowest physically reachable at the
    LOAD seat with the 44 mm lateral grasp shift; verified via 1000-seed
    strict-orient sweeps that found 0 solutions under ~12° tilt).
    APPROACH / RETREAT / LIFT necessarily have slightly higher tilt
    (~28-35°) because they sit 120-150 mm above the seat and need more
    elbow extension; the wrist-lock post-process bounds them under
    ~38°.  Conveyor / vision / bin remain at <1° tilt (unchanged)."""
    import importlib
    importlib.invalidate_caches()
    if "lexium_kinematics" in sys.modules: importlib.reload(sys.modules["lexium_kinematics"])
    if "resolved_poses" in sys.modules:    importlib.reload(sys.modules["resolved_poses"])
    import lexium_kinematics as kin
    import resolved_poses as rp
    import numpy as np
    CONTACT_POSES = [
        "POSE_PLACE_LOAD_FIXTURE", "POSE_RELEASE_LOAD_FIXTURE",
        "POSE_PICK_RIVETED",
    ]
    APPROACH_POSES = [
        "POSE_APPROACH_LOAD_FIXTURE", "POSE_RETREAT_LOAD_FIXTURE",
        "POSE_APPROACH_PICK_RIVETED", "POSE_LIFT_RIVETED",
    ]
    z_world_target = np.array([0.0, 0.0, -1.0])
    bad = []
    for name in CONTACT_POSES:
        q = getattr(rp, name)
        M = kin.fk_world_to_grasp_center(q)
        z_axis = M[:3, :3] @ np.array([0, 0, 1])
        z_err = float(np.linalg.norm(z_axis - z_world_target))
        print(f"  CONTACT  {name:32s} Z_world={tuple(round(v,3) for v in z_axis)}  |Z-err|={z_err:.3f}")
        # V54 contact threshold: 0.40 (~23° tilt) — V54 achieves ~0.34 (~20°).
        if z_err > 0.40:
            bad.append((name, z_err))
    for name in APPROACH_POSES:
        q = getattr(rp, name)
        M = kin.fk_world_to_grasp_center(q)
        z_axis = M[:3, :3] @ np.array([0, 0, 1])
        z_err = float(np.linalg.norm(z_axis - z_world_target))
        print(f"  APPROACH {name:32s} Z_world={tuple(round(v,3) for v in z_axis)}  |Z-err|={z_err:.3f}")
        # V54 approach threshold: 0.65 (~38° tilt) — V54 achieves ~0.51-0.61.
        if z_err > 0.65:
            bad.append((name, z_err))
    assert not bad, "V54 tilt budget exceeded: " + str(bad)


# ---------------------------------------------------------------------------
@case("H. APPROACH/RETREAT vertical: XY matches PLACE / PICK exactly")
def case_h():
    """V54-specific: APPROACH and RETREAT must be vertical above the
    PLACE / PICK seat — same XY, only Z differs."""
    import importlib
    if "lexium_kinematics" in sys.modules: importlib.reload(sys.modules["lexium_kinematics"])
    if "resolved_poses" in sys.modules:    importlib.reload(sys.modules["resolved_poses"])
    import lexium_kinematics as kin
    import resolved_poses as rp
    pairs = [
        ("POSE_PLACE_LOAD_FIXTURE", "POSE_APPROACH_LOAD_FIXTURE", "POSE_RETREAT_LOAD_FIXTURE"),
        ("POSE_PICK_RIVETED",       "POSE_APPROACH_PICK_RIVETED", "POSE_LIFT_RIVETED"),
    ]
    bad = []
    for place, approach, retreat in pairs:
        p_place = kin.fk_world_to_grasp_center(getattr(rp, place))[:3, 3]
        for name in (approach, retreat):
            p = kin.fk_world_to_grasp_center(getattr(rp, name))[:3, 3]
            dxy = ((p[0]-p_place[0])**2 + (p[1]-p_place[1])**2) ** 0.5
            dz  = p[2] - p_place[2]
            print(f"  {name:32s} dXY vs {place}: {dxy*1000:6.2f} mm  dZ: {dz*1000:+7.2f} mm")
            # Tolerance: 20 mm XY drift between vertical poses
            if dxy > 0.020:
                bad.append((name, dxy))
    assert not bad, "Non-vertical approach/retreat: " + str(bad)


# ---------------------------------------------------------------------------
@case("I. Smooth settle — slerp orientation + lerp XY during fall")
def case_i():
    """V54: the object_manager's _init_smooth_settle + slerp settling
    path replaces V53's abrupt position+orientation snap at the moment
    of cradle contact.  Verify the helper exists and that q_slerp
    interpolates correctly."""
    import importlib
    om = importlib.reload(sys.modules["object_manager_node"]) if \
        "object_manager_node" in sys.modules else \
        importlib.import_module("object_manager_node")
    # q_slerp exists and is correct at endpoints + midpoint
    qa = (0.0, 0.0, 0.0, 1.0)
    qb = (1.0, 0.0, 0.0, 0.0)  # Rx(pi)
    assert hasattr(om, "q_slerp"), "object_manager_node.q_slerp missing"
    q_mid = om.q_slerp(qa, qb, 0.5)
    # Midpoint should be Rx(pi/2) = (sin(pi/4), 0, 0, cos(pi/4))
    expected = (math.sin(math.pi/4), 0.0, 0.0, math.cos(math.pi/4))
    diff = max(abs(q_mid[i] - expected[i]) for i in range(4))
    # Quaternion sign ambiguity: accept either q or -q
    diff_neg = max(abs(q_mid[i] + expected[i]) for i in range(4))
    assert diff < 1e-3 or diff_neg < 1e-3, \
        f"q_slerp midpoint wrong: {q_mid} vs {expected}"
    # Endpoint sanity
    assert om.q_slerp(qa, qb, 0.0)[0] == qa[0]
    end = om.q_slerp(qa, qb, 1.0)
    assert max(abs(end[i] - qb[i]) for i in range(4)) < 1e-3
    # _init_smooth_settle exists on the ObjectManager class
    assert hasattr(om.ObjectManager, "_init_smooth_settle"), \
        "ObjectManager._init_smooth_settle missing"
    # Cafi has the new settle fields
    fields = ["settle_start_z", "settle_target_z",
              "settle_start_xy", "settle_target_xy",
              "settle_start_q", "settle_target_q"]
    for f in fields:
        assert f in om.Cafi.__init__.__code__.co_names or \
               hasattr(om.Cafi((0,0,0), (0,0,0,1)), f), \
               f"Cafi missing settle field: {f}"
    print(f"  q_slerp midpoint(0,1) = {tuple(round(v,4) for v in q_mid)} (expected Rx(pi/2))")
    print("  _init_smooth_settle present on ObjectManager")
    print("  Cafi snapshot fields present for smooth-settle")
