    ...
    for step in TRAJ_PICK_CONV:
        ...
        if step == "POSE_PICK_CONVEYOR":
            ...
            log.append("       Margin internal face vs CAFI lateral surface = {:+.5f} m".format(
                int_gap))
            log.append("       ({}{})".format(
                "OUTSIDE the CAFI lateral surface (no penetration)" if int_gap >= 0
                else "INSIDE the CAFI lateral surface by {:.4f} m -- the user's".format(abs(int_gap)),
                "" if int_gap >= 0 else " explicit -39.55 mm shift places the gripper INSIDE the CAFI footprint."))
