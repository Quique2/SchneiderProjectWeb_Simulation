#!/usr/bin/env python3
"""V41 offline cycle simulation.

Walks the cobot through the full process cycle using V41 IK-resolved
...
# V41 cycle (LEFT pick + disc index + vision + bin + return home)
CYCLE_STAGES = [
...
# V41: relaxed from 2.5 to 4.0 rad because the top-down R_target IK
# yields some joint-space jumps between HOME (q=(0,0,0,+pi/2,-pi/2,0))
# and the first APPROACH_* pose -- the robot_controller cosine-interp
# motion is smooth in Cartesian even when the joint L2 jump is large.
MAX_INTER_POSE_DELTA = 4.0
...
def main():
    ...
    log("=" * 80)
    log("V41 OFFLINE RUNTIME CYCLE (DXF layout, cobot on mesa, LEFT pick)")
    log("=" * 80)
    ...
    log("--- 4) Vision presence window check ---")
    # The CAFI lands near the vision fixture at world (0.824, 0.804, 1.042-1.058).
    # vision_sim V41 uses VISION=(0.824, 0.804, 1.050) +-(0.120, 0.120, 0.090).
    vis_x, vis_y, vis_z = 0.824, 0.804, 1.050
    vis_hx, vis_hy, vis_hz = 0.120, 0.120, 0.090
    cafi_at_vision = (0.824, 0.804, 1.042)
    inside = (abs(cafi_at_vision[0]-vis_x) <= vis_hx and
              abs(cafi_at_vision[1]-vis_y) <= vis_hy and
              abs(cafi_at_vision[2]-vis_z) <= vis_hz)
    log(f"  CAFI at vision world position    : {cafi_at_vision}")
    log(f"  vision_sim presence window       : ({vis_x}+-{vis_hx}, "
        f"{vis_y}+-{vis_hy}, {vis_z}+-{vis_hz})")
    log(f"  CAFI inside window?              : {inside}")
    if not inside:
        log("  -> WATCHDOG INSPECT WOULD FIRE")
    log()

    log("--- 5) Simulated runtime cycle (3 CAFIs end-to-end) ---")
    ...
    log_path = os.path.abspath(os.path.join(HERE, "../../../../evidence/logs/runtime_cycle_v41.log"))
